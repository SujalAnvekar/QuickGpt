...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    ...
    from-[#242124] to-[#000000]/30 border-r border-[#80609F]/30 
    backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <button className='flex justify-center items-center w-full py-2 mt-10 text-white bg-linear-to-r
from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer'>
        <span className='mr-2 text-xl'>+</span>New Chat
      </button>

      {/* Search */}
      <div className='flex items-center gap-2 p-3 mt-4 border-gray-400
dark:border-white/10 rounded-md'>
        <img src={assets.search_icon} alt="" className='w-4 not-dark:invert' />
        <input type="text" name="" id="" placeholder='Search' className='text-xs
      ...
      <div className='flex-1 overflow-y-auto mt-3 text-sm space-y-3'>
        {chats.filter((chat) => chat.messages[0] ? chat.messages[0].content.toLowerCase().includes(search.toLowerCase()) :
          chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
            <div
            onClick={()=>{navigate('/');setSelectedChat(chat);setIsMenuOpen(false)}} key={chat._id} className='p-2 px-4 dark:bg-[#57137C]/10 border border-gray-300
  dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
              <div>
                <p className='truncate w-full'>{chat.messages.length > 0 ?
                  chat.messages[0].content.slice(0, 32) : chat.name}</p>
                <p className='text-xs text-gray-500 dark:text-[#B1A6C0]'>{moment(chat.updatedAt).fromNow()}</p>
              </div>
              <img src={assets.bin_icon} alt="" className='hidden group-hover:block
...
rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-gray-400 text-xs'>Purchase credits ro use QuickGPT</p>
        </div>
      </div>
      {/* Dark mode */}
...
rounded-md cursor-pointer group'>
      ...
      <img src={assets.close_icon} alt="" className='absolute top-3 right-3 h-3 w-3
